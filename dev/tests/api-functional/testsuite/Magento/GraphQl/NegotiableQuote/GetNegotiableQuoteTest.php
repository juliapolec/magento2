...
class GetNegotiableQuoteTest extends GraphQlAbstract
{
    ...
    public function testGetNegotiableQuoteExpirationHistory(): void
    {
        ...
        $this->assertNull($response['negotiableQuote']['history'][0]['changes']['expiration']['old_expiration']);
        $this->assertEquals($tomorrow->format('Y-m-d'), $response['negotiableQuote']['history'][0]['changes']['expiration']['new_expiration']);
        // verify expiration change to Never
        $this->assertEquals($tomorrow->format('Y-m-d'), $response['negotiableQuote']['history'][1]['changes']['expiration']['old_expiration']);
        $this->assertEquals('Never', $response['negotiableQuote']['history'][1]['changes']['expiration']['new_expiration']);
    }
    ...
    public function testGetNegotiableQuoteSnapshotById(): void
    {
        ...
        // verify prices
        $this->assertEquals($snapshotQuote->getGrandTotal(), $response['negotiableQuote']['prices']['grand_total']['value']);
        ...
        for ($index = 0; $index < $numUpdatedItems; $index++) {
            $this->assertEquals($quote->getItems()[$index]->getItemId(), $updatedResponse['negotiableQuote']['items'][$index]['id']);
            $this->assertEquals($quote->getItems()[$index]->getQty(), $updatedResponse['negotiableQuote']['items'][$index]['quantity']);
        }
        // verify prices
        $this->assertEquals($quote->getGrandTotal(), $updatedResponse['negotiableQuote']['prices']['grand_total']['value']);
    }
